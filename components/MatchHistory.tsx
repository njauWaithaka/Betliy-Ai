...
type CategoryFilter = 'all' | 'banker' | 'bod' | 'multibet';
type TimeframeFilter = 7 | 14 | 30;

const MatchHistory: React.FC<MatchHistoryProps> = ({ isPremium = false }) => {
  const { data, loading, loadingMore, error, hasMore, loadMore, refresh } = usePremiumHistory();
  ...
  const groupedPicks = useMemo(() => {
    ...
    const processPick = (pick: any, dateStr: string, pkgKey: string) => {
      ...
      // Filter EliteCombo for non-premium users
      if (normalizedPkgKey === 'elitecombo' && !isPremium) return;
  ...
  const filteredPicks = useMemo(() => {
    ...
    return groupedPicks.filter(group => {
      ...
      if (categoryFilter !== 'all') {
        ...
        if (categoryFilter === 'banker' && pkg !== 'dailybanker') return false;
        if (categoryFilter === 'bod' && (pkg !== 'bod' && pkg !== 'dob' && pkg !== 'betoftheday')) return false;
        if (categoryFilter === 'multibet' && group.type !== 'multibet') return false;
      }
  ...
  const stats = useMemo(() => {
    ...
    sortedGroups.forEach(group => {
      ...
      const pkgKey = group.packageKey.toLowerCase();
      const isBod = pkgKey === 'bod' || pkgKey === 'dob' || pkgKey === 'betoftheday';
      const isBanker = pkgKey === 'dailybanker';
      const isMultibet = group.type === 'multibet';

      let includeInChart = true;
      if (chartCategory === 'banker' && !isBanker) includeInChart = false;
      if (chartCategory === 'bod' && !isBod) includeInChart = false;
      if (chartCategory === 'multibet' && !isMultibet) includeInChart = false;
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="bg-emerald-500/10 border border-emerald-500/20 rounded-[1.5rem] sm:rounded-[2.5rem] p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4"
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm sm:text-lg font-black text-white uppercase italic tracking-tight">ELITE PERFORMANCE VAULT</h3>
            <p className="text-[9px] sm:text-[11px] font-bold text-emerald-400 uppercase tracking-widest">Real Results. Verified Growth. Zero Guesswork.</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6">
          <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/10">
            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">🔒 Premium Members Only Data</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/10">
            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">⏱ Updated Daily</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/10">
            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">📊 No Edited History</span>
          </div>
        </div>
        <div className="flex items-center gap-6">
          <div className="text-center">
            <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Verified Wins</p>
            <p className="text-xl font-black text-emerald-400 italic">{stats?.wins || 0}</p>
          </div>
          <div className="h-8 w-[1px] bg-white/10" />
          <div className="text-center">
            <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Bets Tracked</p>
            <p className="text-xl font-black text-white italic">{stats?.totalBets || 0}</p>
          </div>
        </div>
      </motion.div>

      {/* Narrative Layer (AI Insight) */}
      {stats && (
        <motion.div
          initial={{ x: -20, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ delay: 0.2 }}
          className="bg-slate-900/60 border-l-4 border-emerald-500 rounded-r-[1.5rem] p-6 space-y-3 shadow-2xl relative overflow-hidden"
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <h4 className="text-xs font-black text-white uppercase tracking-widest">AI Intelligence Insight</h4>
          </div>
          <div className="space-y-2 relative z-10">
            <p className="text-xs sm:text-sm font-medium text-slate-300 leading-relaxed">
              This cycle shows <span className="text-emerald-400 font-bold">strong consistency</span> in low-risk markets. 
              <span className="text-white"> {stats.bestCategory}</span> picks contributed <span className="text-emerald-400 font-bold">{stats.bestCategoryContribution}%</span> of total profit. 
              Volatility remains low → <span className="text-white">stable growth expected</span> for the current market conditions.
            </p>
            <div className="flex gap-4 pt-2">
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Stable Growth Mode</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Low Volatility</span>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4">
          <h2 className="text-4xl sm:text-8xl font-black text-white italic uppercase tracking-tighter leading-[0.85]">
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
          {[
            { label: "ROI (%)", value: `+${stats.roi}%`, icon: TrendingUp, color: "text-emerald-400", highlight: true, subIcon: "📈" },
            { label: "Total Profit", value: `+$${stats.profit}`, icon: DollarSign, color: "text-white", highlight: true, subIcon: "💰" },
            { label: "Win Rate", value: `${stats.winRate}%`, icon: Percent, color: "text-amber-400", subIcon: "🎯" },
            { label: "Avg Odds", value: stats.avgOdds, icon: Target, color: "text-blue-400", subIcon: "📊" },
            { label: "Top Category", value: stats.bestCategory, icon: Trophy, color: "text-purple-400", subIcon: "🏆" }
          ].map((item, i) => (
            <motion.div 
              key={i} 
              ...
              className={`bg-slate-900/40 border ${item.highlight ? 'border-emerald-500/30' : 'border-white/5'} rounded-[1.5rem] sm:rounded-[2.5rem] p-4 sm:p-6 relative overflow-hidden group hover:border-white/10 transition-all shadow-2xl`}
            >
              <div className={`absolute -right-4 -bottom-4 sm:-right-6 sm:-bottom-6 opacity-[0.03] group-hover:opacity-[0.07] transition-opacity ${item.highlight ? 'text-emerald-500' : 'text-white'}`}>
              ...
              <p className={`text-2xl sm:text-4xl font-black italic tracking-tighter ${item.color} ${item.highlight ? 'drop-shadow-[0_0_15px_rgba(52,211,153,0.3)]' : ''}`}>
                {item.value}
              </p>
              {item.highlight && (
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 p-1 bg-slate-950/50 rounded-full overflow-x-auto no-scrollbar">
              {(['all', 'bod', 'banker', 'multibet'] as CategoryFilter[]).map((cat) => (
                <button
                  key={cat}
      ...
      {stats && (
        ...
        >
          {stats.categoryBreakdown.map((cat) => (
            <div key={cat.name} className="bg-slate-900/20 border border-white/5 rounded-[1.5rem] p-4 flex items-center justify-between">
              <div>
                <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">{cat.name}</p>
                <p className="text-sm font-black text-white italic">{cat.winRate}% Win | +${cat.profit}</p>
              </div>
              <div className="text-right">
                <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest">Signals</p>
                <p className="text-sm font-black text-slate-400">{cat.total}</p>
              </div>
            </div>
          ))}
        </motion.div>
      ...
      >
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 justify-center">
            ...
            {/* Category Chips */}
            <div className="flex items-center gap-1 p-1 bg-slate-950/50 rounded-full">
              {(['all', 'banker', 'bod', 'multibet'] as CategoryFilter[]).map((c) => (
                <button
                  key={c}
            ...
            {filteredPicks.map((group) => {
              ...
              return (
                <motion.div 
                  ...
                  transition={{ delay: 0.1 }}
                  className={`group bg-slate-900/20 border ${isWin ? 'border-emerald-500/20 hover:border-emerald-500/40' : isLoss ? 'border-rose-500/20 hover:border-rose-500/40' : 'border-white/5 hover:border-white/10'} rounded-[1.5rem] sm:rounded-[3rem] p-4 sm:p-8 hover:bg-slate-900/40 transition-all relative overflow-hidden shadow-xl`}
                >
                  {/* Glow Effect */}
                  <div className={`absolute -right-20 -top-20 w-40 h-40 blur-[80px] opacity-20 pointer-events-none ${isWin ? 'bg-emerald-500' : isLoss ? 'bg-rose-500' : 'bg-amber-500'}`} />

                  {/* Header Info */}
                  <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-8">
                    <div className="flex items-center gap-2 sm:gap-4">
                      <div className={`px-3 sm:px-6 py-1.5 sm:py-2 rounded-full text-[8px] sm:text-[11px] font-black uppercase tracking-widest shadow-lg ${
                        ...
                        Verified Alpha Signals: {group.packageName}
                      </div>
                      <div className="flex items-center gap-2 text-slate-500">
                        <Clock size={12} />
                        <span className="text-[9px] sm:text-[11px] font-black uppercase tracking-widest italic">{group.date}</span>
                      </div>
                      {group.type === 'multibet' && (
                        <span className="px-2 py-1 rounded-full bg-white/5 text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest border border-white/5">
                    ...
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className={`flex flex-col items-end gap-1 px-4 sm:px-8 py-2 sm:py-3 rounded-[1rem] sm:rounded-[1.5rem] text-[10px] sm:text-[14px] font-black uppercase tracking-widest shadow-2xl transition-all ${
                        isWin ? 'bg-white text-emerald-600 border border-emerald-200 drop-shadow-[0_0_15px_rgba(52,211,153,0.4)]' : 
                        isLoss ? 'bg-rose-500/10 text-rose-500 border border-rose-500/20' : 
                        'bg-amber-500/10 text-amber-500 border border-amber-500/20'
                      }`}>
                        <div className="flex items-center gap-2">
                          {isWin ? <CheckCircle2 size={16} /> : isLoss ? <XCircle size={16} /> : <HelpCircle size={16} />}
                          {isWin ? `WON (${group.profit})` : isLoss ? `LOSS (${group.profit})` : 'PENDING'}
                        </div>
                        {group.type === 'single' && group.picks[0].ft_score && (
                          <span className="text-[8px] sm:text-[10px] opacity-70">Score: {group.picks[0].ft_score}</span>
                        )}
                      </div>
                      {group.type === 'multibet' && (
                        <button 
                          onClick={(e) => { e.stopPropagation(); toggleGroup(group.id); }}
                          className="p-2 sm:p-3 hover:bg-white/5 rounded-full transition-colors text-slate-400 border border-white/5"
                        >
                    ...
                    {isExpanded && (
                      ...
                      >
                        {group.picks.map((pick: any, idx: number) => {
                          ...
                          return (
                            <div key={pickKey} className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 sm:gap-10 pb-6 sm:pb-10 last:pb-0 border-b last:border-0 border-white/5">
                              <div className="flex items-center gap-4 sm:gap-8 min-w-0 flex-1">
                                <div className="flex -space-x-4 sm:-space-x-6 shrink-0">
                                  <div className="w-12 h-12 sm:w-20 sm:h-20 rounded-2xl sm:rounded-[2rem] bg-slate-950 border border-white/10 flex items-center justify-center p-2 sm:p-4 shadow-2xl relative z-10">
                                    {pick.home_logo ? <img src={pick.home_logo} alt={pick.home} className="w-full h-full object-contain" referrerPolicy="no-referrer" /> : <Target size={24} className="text-slate-800" />}
                                  </div>
                                  <div className="w-12 h-12 sm:w-20 sm:h-20 rounded-2xl sm:rounded-[2rem] bg-slate-950 border border-white/10 flex items-center justify-center p-2 sm:p-4 shadow-2xl">
                                    {pick.away_logo ? <img src={pick.away_logo} alt={pick.away} className="w-full h-full object-contain" referrerPolicy="no-referrer" /> : <Target size={24} className="text-slate-800" />}
                                  </div>
                                </div>
                                
                                <div className="min-w-0 flex-1 space-y-1 sm:space-y-3">
                                  <div className="flex items-center gap-3">
                                    <span className="text-[9px] sm:text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1">
                                      <MapPin size={10} /> {pick.league}
                                    </span>
                                    <span className={`px-2 py-0.5 rounded-md text-[8px] sm:text-[10px] font-black uppercase tracking-widest ${riskColor}`}>
                                      {riskLevel}
                                    </span>
                                  </div>
                                  <h4 className="text-xl sm:text-4xl font-black text-white uppercase italic tracking-tighter leading-none">
                                    <span className="sm:hidden">{shortenTeamName(pick.home)} <span className="text-slate-700 mx-1">VS</span> {shortenTeamName(pick.away)}</span>
                                    <span className="hidden sm:inline">{pick.home} <span className="text-slate-700 mx-2">VS</span> {pick.away}</span>
                                  </h4>
                                  <div className="flex flex-wrap items-center gap-3 sm:gap-6 text-[10px] sm:text-[13px] font-bold uppercase tracking-widest">
                                    <span className="text-emerald-400">Market: {pick.tip || pick.pick || pick.prediction || 'Verified Pick'}</span>
                                    {pick.ft_score && (
                                      <span className="text-slate-400 bg-white/5 px-2 py-0.5 rounded-md">Score: {pick.ft_score}</span>
                                    )}
                                    {pick.kickoff && (
                                      <span className="text-slate-500 flex items-center gap-1"><Clock size={12} /> {pick.kickoff}</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                              ...
                              {group.type === 'single' && (
                                <div className="grid grid-cols-3 sm:flex sm:items-center sm:justify-end gap-4 sm:gap-12 shrink-0 bg-slate-950/30 lg:bg-transparent p-4 sm:p-0 rounded-2xl lg:rounded-none border border-white/5 lg:border-0">
                                  <div className="text-left sm:text-right space-y-1 sm:space-y-2">
                                    <p className="text-[8px] sm:text-[10px] font-black text-slate-600 uppercase tracking-widest">Odds</p>
                                    <p className="text-lg sm:text-3xl font-black text-white italic tracking-tighter">@ {pick.odds}</p>
                                  </div>
                                  <div className="text-left sm:text-right space-y-1 sm:space-y-2">
                                    <p className="text-[8px] sm:text-[10px] font-black text-slate-600 uppercase tracking-widest">Confidence</p>
                                    <p className="text-lg sm:text-3xl font-black text-emerald-400 italic tracking-tighter">{pick.confidence}%</p>
                                  </div>
                                  <div className="text-left sm:text-right space-y-1 sm:space-y-2">
                                    <p className="text-[8px] sm:text-[10px] font-black text-slate-600 uppercase tracking-widest">Profit</p>
                                    <p className={`text-lg sm:text-3xl font-black italic tracking-tighter ${
                  ...
                  {/* Multibet Footer */}
                  {group.type === 'multibet' && (
                    <div className="mt-6 sm:mt-12 pt-6 sm:pt-10 border-t border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-6 sm:gap-10">
                      <div className="grid grid-cols-2 sm:flex sm:items-center sm:justify-start gap-8 sm:gap-16">
                        <div className="space-y-1 sm:space-y-2">
                          <p className="text-[9px] sm:text-[11px] font-black text-slate-500 uppercase tracking-widest">Total Multibet Odds</p>
                          <p className="text-2xl sm:text-5xl font-black text-white italic tracking-tighter">@ {group.odds}</p>
                        </div>
                        <div className="space-y-1 sm:space-y-2">
                          <p className="text-[9px] sm:text-[11px] font-black text-slate-500 uppercase tracking-widest">Avg Confidence</p>
                          <p className="text-2xl sm:text-5xl font-black text-emerald-400 italic tracking-tighter">{group.confidence}%</p>
                        </div>
                      </div>
                      <div className="text-left sm:text-right space-y-1 sm:space-y-2 bg-slate-950/30 sm:bg-transparent p-4 sm:p-0 rounded-2xl sm:rounded-none border border-white/5 sm:border-0">
                        <p className="text-[9px] sm:text-[11px] font-black text-slate-500 uppercase tracking-widest">Total Profit</p>
                        <p className={`text-3xl sm:text-6xl font-black italic tracking-tighter ${
