  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        {isFree ? t('card.free', 'Free Pick') : (badge || title)}
      </div>
      ...
      <div className={cn(
        ...
        {isFree ? t('card.free', 'Free Pick') : t('card.premiumPick', 'Premium Pick')}
      </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-col">
                ...
                <span className={cn(
                  ...
                  {isFree ? tip : t('card.premiumOnly', 'Premium Only')}
                </span>
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">{t('card.confidence', 'CONFIDENCE')}</span>
            ...
            <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/5">
              {isFree ? (
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">{t('card.freeSampleUnlocked', 'FREE DAILY SAMPLE UNLOCKED')}</span>
              ) : (
                <>
                  <Lock size={10} className={cn(colorClasses[color as keyof typeof colorClasses].split(' ')[0])} />
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">{footerText || t('card.unlockToView', 'UNLOCK TO VIEW FULL PREDICTION')}</span>
                </>
              )}
            </div>
          </div>
    ...
    >
      ...
      {bestValue && (
        <div className="absolute top-4 right-6 flex items-center gap-1.5 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
          ...
          <span className="text-[8px] font-black text-emerald-400 uppercase tracking-widest">{t('vip.bestValue', 'BEST VALUE')}</span>
        </div>
      )}
      ...
      <div className="mb-6">
        ...
        <div className="flex items-end gap-3">
          ...
          <span className="text-[10px] font-black text-[#00FFA3] uppercase tracking-widest mb-1">/ {t('vip.month', 'Month')}</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {isPremium && (
          <div className="space-y-2">
            <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest animate-pulse">
              {t('vip.limitedTimePrice', 'LIMITED-TIME PRICE — INCREASING SOON')}
            </p>
            <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">
              {t('vip.bestValueDesc', 'BEST VALUE FOR USERS WHO WANT THE FULL AI SYSTEM')}
            </p>
          </div>
        )}
        ...
        <div className="text-center">
          <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">
            {isPremium ? t('vip.unlockAll6', 'UNLOCK ALL 6 LOCKED PREMIUM SECTIONS') : t('vip.unlock2Core', 'UNLOCK 2 CORE DAILY SIGNALS')}
          </span>
        </div>
      </div>
    </motion.div>
        ...
        <div className="flex-1 px-6 pt-8 pb-32 space-y-10">
          ...
          >
            ...
            <div className="relative z-10 space-y-6">
              ...
              >
                ...
                <span className="text-[10px] font-black text-white uppercase tracking-[0.2em]">{t('vip.picksLive', "TODAY'S PICKS ARE LIVE")}</span>
              </motion.div>
              ...
              >
                <span>{t('vip.missingHeader1', "YOU'RE MISSING")}</span>
                <span className="text-[#00FFA3]">{t('vip.missingHeader2', "TODAY'S WINNING")}</span>
                <span className="flex items-center justify-center gap-2">{t('vip.missingHeader3', "AI PICKS")} <Lock size={24} className="text-[#00FFA3]" /></span>
              </motion.h1>
              ...
              <div className="space-y-3 pt-2 w-full max-w-sm sm:max-w-md mx-auto">
                ...
                >
                  <div className="flex flex-col justify-center space-y-0.5 sm:space-y-1">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{t('stats.winRate', 'WIN RATE')}</span>
                    ...
                    <span className="text-[8px] font-medium text-slate-500 lowercase">{t('stats.last50Signals', 'last 50 signals')}</span>
                  </div>

                  <div className="flex flex-col justify-center space-y-0.5 sm:space-y-1 px-1">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{t('stats.totalProfit', 'NET PROFIT')}</span>
                  ...
                  <div className="flex flex-col justify-center space-y-0.5 sm:space-y-1">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{t('vip.lastResults', 'LAST RESULTS')}</span>
                    ...
                    <span className="text-[8px] font-medium text-slate-500 lowercase">{t('vip.verifiedDaily', 'verified daily')}</span>
                  </div>
                </motion.div>
              </div>
            </div>
          ...
          >
            <div className="flex items-center justify-between px-1">
              <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">{t('vip.todaysSignals', "TODAY'S SIGNALS")}</h3>
              <span className="text-xs font-black text-[#00FFA3] uppercase tracking-[0.2em]">{t('vip.signalsCount', '1 FREE | 6 LOCKED')}</span>
            </div>
            ...
            >
              ...
              <div className="space-y-1">
                <p className="text-[10px] font-black text-orange-500 uppercase tracking-widest">{t('vip.lockedNoticeTitle', '6 HIGH-CONFIDENCE PICKS LOCKED TODAY')}</p>
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-tight">{t('vip.lockedNoticeSub', 'AI ELITE ACCESS UNLOCKS THE FULL BOARD INSTANTLY')}</p>
              </div>
            </motion.div>
            ...
            <div className="space-y-3">
              ...
              <LockedPickCard 
                title={t('card.botdTitle', 'PREMIUM AI BET OF THE DAY')} 
                confidence={91} 
                odds="SCANNING" 
                icon={Zap} 
                color="amber"
                footerText={t('card.eliteAnalysisLocked', 'ELITE ANALYSIS LOCKED')}
              />
              <LockedPickCard 
                title={t('combo.title', 'PREMIUM AI ELITE COMBO')} 
                confidence={75} 
                odds="SCANNING" 
                icon={Flame} 
                color="teal"
                footerText={t('combo.revealStrategy', 'REVEAL MULTI-BET STRATEGY')}
              />
              <LockedPickCard 
                title={t('card.verifiedPicksTitle', 'AI VERIFIED PICKS')} 
                confidence={88} 
                odds="SCANNING" 
                icon={Target} 
                color="blue"
                footerText={t('card.viewVerifiedPicks', 'VIEW VERIFIED HIGH-CONFIDENCE PICKS')}
                badge={t('card.verifiedPicksTitle', 'AI VERIFIED PICKS')}
              />
            </div>
          </motion.div>
          ...
          >
            <div className="absolute top-0 right-0 p-4">
              ...
              >
                ...
                <span className="text-[8px] font-black text-rose-500 uppercase tracking-widest">{t('vip.livePerf', 'LIVE PERFORMANCE')}</span>
              </motion.div>
            </div>

            <div className="space-y-8 relative z-10">
              <div className="space-y-1">
                <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter">{t('vip.picksLive', "TODAY'S AI PICKS ARE LIVE")}</h3>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">{t('vip.lastResults', 'LAST RESULTS')}</p>
              </div>
              ...
              <div className="space-y-1">
                ...
                >
                  8/10 {t('stats.won', 'Won')}
                </motion.div>
                <p className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em]">{t('vip.verifiedDailyResults', 'VERIFIED DAILY RESULTS')}</p>
              </div>

              <div className="space-y-1">
                <p className="text-[10px] font-black text-emerald-500/60 uppercase tracking-[0.4em]">{t('stats.totalProfit', 'NET PROFIT')}</p>
                <motion.div 
                  initial={{ y: 10, opacity: 0 }}
                  whileInView={{ y: 0, opacity: 1 }}
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <span className="px-2 py-0.5 bg-white/5 border border-white/10 rounded text-[9px] font-black text-slate-500 uppercase tracking-widest">{t('vip.verified', 'VERIFIED')}</span>
              </div>
              ...
              >
                <div className="text-xl sm:text-2xl font-black text-white italic tracking-tighter">80% {t('stats.winRate', 'Win Rate')}</div>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{t('stats.last50SignalsUpper', 'LAST 50 AI SIGNALS')}</p>
              </motion.div>
            </div>
          </motion.div>
          ...
          <div className="space-y-8">
            ...
            >
              <PricingTier 
                isPremium
                badge={t('vip.mostPopular', 'MOST POPULAR')}
                bestValue
                title={t('vip.eliteTitle', 'AI ELITE ACCESS')}
                subtext={t('vip.unlockFullSystem', 'UNLOCK THE FULL AI SYSTEM.')}
                price="$29.99"
                originalPrice="$49.99"
                features={[
                  t('card.botdTitle', 'BET OF THE DAY'),
                  t('combo.title', 'ELITE COMBO'),
                  t('card.verifiedPicksTitle', 'AI VERIFIED PICKS')
                ]}
                supportingLine={t('vip.supportingLine', 'MULTIPLE STRATEGIES. HIGHER PROFIT POTENTIAL. MORE WINNING OPPORTUNITIES.')}
                ctaText={t('vip.unlockFullCta', 'UNLOCK FULL AI ACCESS')}
          ...
          >
            <h3 className="text-lg font-black text-[#00FFA3] italic uppercase tracking-tighter leading-tight max-w-[280px] mx-auto">
              {t('vip.trustBanner', 'MOST USERS CHOOSE ELITE FOR FULL ACCESS AND HIGHER WIN POTENTIAL')}
            </h3>
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">
              <div className="flex items-center gap-2"><Check size={14} className="text-[#00FFA3]" /> {t('vip.instantAccess', 'INSTANT ACCESS')}</div>
              <div className="flex items-center gap-2"><Check size={14} className="text-[#00FFA3]" /> {t('vip.securePayment', 'SECURE PAYMENT')}</div>
              <div className="flex items-center gap-2"><Check size={14} className="text-[#00FFA3]" /> {t('vip.cancelAnytime', 'CANCEL ANYTIME')}</div>
            </div>
          </motion.div>
