...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-slate-950/90 backdrop-blur-xl p-4 animate-in fade-in duration-300">
      <div className="relative w-full max-w-[400px] bg-[#020617] border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col p-8 space-y-8">
        <button onClick={onClose} className="absolute top-6 right-6 p-2 text-slate-500 hover:text-white transition-all bg-white/5 rounded-full">
          <X size={20} />
        </button>
        ...
        <div className="space-y-4">
          {authService.isTelegramMiniApp() ? (
            <button
              ...
              disabled={loading}
              className="w-full py-4 bg-[#24A1DE] text-white rounded-2xl flex items-center justify-center gap-3 font-black uppercase italic text-sm hover:bg-[#208ec4] transition-all active:scale-95 shadow-[0_0_20px_rgba(36,161,222,0.3)] disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-4 bg-white text-slate-950 rounded-2xl flex items-center justify-center gap-3 font-black uppercase italic text-sm hover:bg-slate-200 transition-all active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.1)] disabled:opacity-50"
            >
