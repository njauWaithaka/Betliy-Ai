...
const EliteComboCard: React.FC<EliteComboCardProps> = ({ picks, isPremium, onUnlock, onPlaceBet }) => {
  ...
  return (
    ...
    >
      ...
      {!isPremium && (
        <div className="absolute inset-0 z-50 bg-slate-950/60 backdrop-blur-[6px] flex flex-col items-center justify-center p-8 text-center rounded-[2rem] border border-white/5">
          ...
          <h3 className="text-xl font-black text-white italic uppercase tracking-tighter mb-2">🔒 ELITE COMBO LOCKED</h3>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest max-w-xs mb-6 leading-relaxed">
            Unlock today's high-odds elite combo and access the full neural breakdown for all picks.
          </p>
          <button 
            onClick={onUnlock}
            className="px-8 py-3 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-xl font-black uppercase italic text-xs transition-all shadow-lg shadow-emerald-500/20 touch-target"
          >
            Unlock Now
          </button>
        </div>
      ...
      {/* Header Card */}
      <div className="bg-gradient-to-br from-emerald-500/20 via-slate-900/90 to-slate-950 border border-emerald-500/30 p-6 rounded-[2rem] relative overflow-hidden shadow-[0_0_30px_rgba(16,185,129,0.15)]">
