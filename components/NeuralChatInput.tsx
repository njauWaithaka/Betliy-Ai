...
const QUICK_PACKS = [
  { 
    id: 'alphasignals', 
    labelKey: 'card.freePicksTitle',
    defaultLabel: '🎁 FREE PICKS', 
    descKey: 'card.dailyFreePicksDesc',
    defaultDesc: '1–3 AI-POWERED PICKS DAILY.',
    icon: Zap, 
  ...
  { 
    id: 'botd', 
    labelKey: 'card.botdTitle',
    defaultLabel: '🎯 BET OF THE DAY', 
    descKey: 'card.botdDesc',
    defaultDesc: "TODAY'S STRONGEST AI PREDICTION.",
    icon: Zap, 
  ...
  { 
    id: 'toppicks', 
    labelKey: 'card.verifiedPicksTitle',
    defaultLabel: '🤖 AI VERIFIED PICKS', 
    descKey: 'card.verifiedPicksDesc',
    defaultDesc: '6–10 PREMIUM AI PICKS WITH DETAILED ANALYSIS.',
    icon: Trophy, 
  ...
  { 
    id: 'elitecombo', 
    labelKey: 'combo.title',
    defaultLabel: '💎 ELITE COMBO', 
    descKey: 'combo.desc',
    defaultDesc: "TODAY'S PREMIUM ACCUMULATOR BUILT BY AI.",
    icon: Flame, 
      ...
      <div className="relative bg-slate-950/90 border border-emerald-500/20 p-4 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl space-y-4 sm:space-y-8 backdrop-blur-2xl shadow-[0_0_30px_rgba(16,185,129,0.1)]">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 px-1 sm:px-2">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h3 className="text-[8px] sm:text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] sm:tracking-[0.4em] italic leading-none">{t('hub.title', 'BETLIFY COMMAND HUB')}</h3>
            </div>
          </div>
          <div className="relative group/badge self-start sm:self-auto">
            ...
            <div className="relative flex items-center gap-1.5 sm:gap-2 px-2 py-1 sm:px-2.5 sm:py-1.5 bg-gradient-to-r from-emerald-600 to-emerald-500 text-slate-950 text-[7px] sm:text-[9px] font-black uppercase italic rounded-lg sm:rounded-xl shadow-lg border border-white/20">
              ...
              <span className="tracking-tighter">{t('nav.terminal', 'MARKET ANALYSIS')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:gap-4 px-1 sm:px-2">
           {QUICK_PACKS.map((pack, idx) => {
             const label = t(pack.labelKey, pack.defaultLabel);
             const desc = t(pack.descKey, pack.defaultDesc);
             return (
               <motion.button 
                 key={pack.id} 
                 ...
                 type="button" 
                 disabled={loading} 
                 onClick={() => onQuickAction?.(pack.defaultLabel, pack.isFree)} 
                 className={cn(
                   "flex items-center justify-between p-3 sm:p-5 bg-slate-900/60 border rounded-xl sm:rounded-3xl hover:bg-slate-800/80 hover:shadow-[0_0_25px_rgba(16,185,129,0.2)] transition-colors group/chip relative overflow-hidden disabled:opacity-50 text-left cursor-pointer",
                   pack.border
                 )}
               >
